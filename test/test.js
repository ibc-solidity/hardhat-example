...
describe('IBCHandlerTest', async function () {
    ...
    before(async function () {
        ...
        ibcHandler = await deployIBC(await deployer.getAddress());
    })
    ...
    it('should register a mock client', async function () {
        const mockClient = await ethers.deployContract("MockClient", [ibcHandler.target]);
    ...
    it('should bind the MockApp to a port', async function () {
        const mockApp = await ethers.deployContract("IBCMockApp", [ibcHandler.target]);
